...
class FundNavAnalyticsApiInternalTestCase(APITest):
    def setUp(self):
        super(FundNavAnalyticsApiInternalTestCase, self).setUp()
    ...
    def test_get_internal_fund_detail_success(self):
        url = reverse("api_internal:fund-price-analytics", args=["VCAMBF"])
    ...
    def test_get_internal_fund_detail_have_no_performance(self):
        url = reverse("api_internal:fund-price-analytics", args=["VSAF"])
        response = self.client.get(url)
