...
class FundPriceAnalyticsApiInternalTestCase(APITest):
    def setUp(self):
        super(FundPriceAnalyticsApiInternalTestCase, self).setUp()
        ...
        first_datetime = datetime(2021, 8, 7, 8, 8, 8, tzinfo=tzoffset(None, 25200))
        ...
        upto_now = datetime.now(tz=tzoffset(None, 25200))
        nav_daily = MutualFundNavDailyFactory(
            symbol="VCAMBF",
            nav=15000.0,
            date=upto_now,
    ...
    def test_get_internal_fund_detail_success(self):
        url = reverse("api_internal:mutual-fund-price-detail", args=["VCAMBF"])
        url = url
    ...
    def test_get_internal_fund_detail_have_no_performance(self):
        url = reverse("api_internal:mutual-fund-price-detail", args=["VSAF"])
        url = url
        response = self.client.get(url)
