...
from tests.factories.collection import WatchlistCollectionFactory, MetricFactory


class WatchlistCollectionApiTestCase(APITest):

    def setUp(self):
        super(WatchlistCollectionApiTestCase, self).setUp()
        ...
        self.collection = WatchlistCollectionFactory(
        ...
        self.collection2 = WatchlistCollectionFactory(
            title='Test collection', product_type='stock', status=ContentStatusEnum.DRAFT,
            field_params=['name', 'symbol', 'price', 'volume'],
            filter_params=[{'name': 'volume', 'operator': 'gt', 'value': 3000},
        ...
        self.collection3 = WatchlistCollectionFactory(
            title='Test collection', product_type='stock', status=ContentStatusEnum.PUBLISHED,
            field_params=['name', 'symbol', 'price', 'volume'],
            filter_params=[{'name': 'volume', 'operator': 'gt', 'value': 3000},
                           {'name': 'exchange', 'operator': 'eq', 'value': 'hose'}],
    ...
    def test_get_watchlist_collections(self):
        url = reverse('api:watchlist-collections-list')
        response = self.client.get(url)

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['items'][0]['title'], self.collection.title)

        self.assertEqual(response.data['items'][0]['items'][0]['symbol'], 'MBB')

        self.assertEqual(response.data['items'][0]['items'][0]['metrics'][0]['key'], 'name')
        self.assertEqual(response.data['items'][0]['items'][0]['metrics'][0]['label'], 'Name')
        self.assertEqual(response.data['items'][0]['items'][0]['metrics'][0]['value'], 'Ngan hang quan doi')
        self.assertEqual(response.data['items'][0]['items'][0]['metrics'][2]['key'], 'price')
        self.assertEqual(response.data['items'][0]['items'][0]['metrics'][2]['label'], 'Price')
        self.assertEqual(response.data['items'][0]['items'][0]['metrics'][2]['value'], 27)

        self.assertEqual(response.data['items'][0]['items'][1]['symbol'], 'VCB')
        self.assertEqual(response.data['items'][0]['items'][1]['metrics'][0]['key'], 'name')
        self.assertEqual(response.data['items'][0]['items'][1]['metrics'][0]['label'], 'Name')
        self.assertEqual(response.data['items'][0]['items'][1]['metrics'][0]['value'], 'Ngan hang VCB')
        self.assertEqual(response.data['items'][0]['items'][1]['metrics'][2]['key'], 'price')
        self.assertEqual(response.data['items'][0]['items'][1]['metrics'][2]['label'], 'Price')
        self.assertEqual(response.data['items'][0]['items'][1]['metrics'][2]['value'], 28)
