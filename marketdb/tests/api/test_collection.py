...
class CollectionApiTestCase(APITest):

    def setUp(self):
        super(CollectionApiTestCase, self).setUp()
        ...
        self.collection3 = CollectionFactory(
            title='Test collection', product_type='stock', status=ContentStatusEnum.PUBLISHED,
            field_params=['name', 'symbol', 'price', 'volume'],
            filter_params=[{'name': 'volume', 'operator': 'gt', 'value': 3000},
                           {'name': 'exchange', 'operator': 'eq', 'value': 'hose'}],
    ...
    def test_get_collection_detail(self):
        url = reverse('api:collections-detail', args=[self.collection.id])
        response = self.client.get(url)

        self.assertEqual(response.data['title'], self.collection.title)
        self.assertEqual(response.data['product_type'], self.collection.product_type)
        self.assertEqual(response.data['field_params'], self.collection.field_params)
        self.assertEqual(response.data['filter_params'], self.collection.filter_params)
        self.assertEqual(response.data['sort_params'], self.collection.sort_params)

    def test_get_collection_detail_not_found(self):
        url = reverse('api:collections-detail', args=[self.collection2.id])
        response = self.client.get(url)

        self.assertEqual(response.status_code, 404)

    def test_get_collection_app(self):
        url = reverse('api:collections-by-content-type', args=['app'])
        response = self.client.get(url)

        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data['items']), 1)
        self.assertEqual(response.data['items'][0]['title'], self.collection3.title)
